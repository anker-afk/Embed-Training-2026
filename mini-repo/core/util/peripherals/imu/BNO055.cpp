...
void BNO055 :: init() noexcept {
    ...
    _i2c.write(chip_addr , config_mode, 2, false);
    ThisThread :: sleep_for(19ms);

    char dof_mode[]  = {0x3D, 0x0C};
    _i2c.write(chip_addr , dof_mode, 2, false);
...
void BNO055 :: get_accel(BNO055_VECTOR_TypeDef *la) {
    ...
    char accel[] = {0x08};
    int16_t x,y,z;
    int writeResult = _i2c.write(chip_addr , accel, 1, true);
    ...
    if (!writeResult)  {
        ...
        la->x = (double)x / 100; // 100LSB per m/s^2
        la->y = (double)y / 100; // 100LSB per m/s^2
        la->z = (double)z / 100; // 100LSB per m/s^2
    }
    else{
        cantReadDataCount ++;
    }
...
void BNO055 :: get_gyro(BNO055_VECTOR_TypeDef *gr){
    ...
    _i2c.write(chip_addr , gyro, 1, true);
    _i2c.read(chip_addr, dt, 6, false);
    ...
    gr -> x = (double)x / 16.0; //16 LSB/dps
    gr -> y = (double)y / 16.0; //16 LSB/dps
    gr -> z = (double)z / 16.0; //16 LSB/dps
}
//read override
...
void BNO055 :: change_fusion_mode(uint8_t mode){
    ...
    _i2c.write(chip_addr , change, 2, false);
    ThisThread :: sleep_for(19ms);

    //then change to the mode
    char change_to_mode[] = {0x3D, (char)mode};
    _i2c.write(chip_addr , change_to_mode, 2, false);
    ThisThread :: sleep_for(9ms);
}
